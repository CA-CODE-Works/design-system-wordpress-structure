...
$cagov_ds_structure_search_nonce    = wp_create_nonce( 'cagov_ds_structure_google_cse' );
$cagov_ds_structure_search_verified = isset( $cagov_ds_structure_search_nonce ) && wp_verify_nonce( sanitize_key( $cagov_ds_structure_search_nonce ), 'cagov_ds_structure_google_cse' );
$cagov_ds_structure_search_keyword  = isset( $_GET['q'] ) ? sanitize_text_field( wp_unslash( $_GET['q'] ) ) : '';
...
<div class="search-container search-container--small grid-search">
	<form class="site-search" action="<?php print esc_url( site_url( 'serp' ) ); ?>">
		...
		<input type="text" id="q" name="q" value="<?php print esc_attr( $cagov_ds_structure_search_keyword ); ?>" aria-labelledby="SearchInput" placeholder="Search this website" class="search-textfield">
